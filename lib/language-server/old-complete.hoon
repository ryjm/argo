...
++  search-prefix
  ...
  |=  [id=cord *]
  ^-  ?(%.y %.n)
  ~?  >  =(sid (end [3 (met 3 sid)] id))  prefix+[sid id]
...
++  get-id
  ...
  ::  cuts the first pos chars from txt and searches that for seek.
...
++  exact-list-hoon
  |=  [sut=type gen=hoon]
  ^-  (unit (option type))
  %+  biff  (find-type-mule sut gen)
  |=  [id=term typ=type]
  (search-exact id (get-identifiers typ))
::
++  exact-list-tape
  |=  [sut=type pos=@ud code=tape]
  ^-  (each (unit (option type)) [row=@ col=@])
  ~?  >  debug  %start-magick
  =/  magicked  txt:(insert-magic pos code)
